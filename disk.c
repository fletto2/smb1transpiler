 ...
 * and the only transform is the DOS 3.3 physical-to-logical skew.  So the whole
 * job is "put this page at (track, physical sector)".  The port's own RWTS
 * reads by physical position with a +2 interleave, which is why every blob is
 * laid down in physical order and the skew is applied on the way into the file.
 *
...
static void put_phys(struct disk *d, int track, int phys,
...
static long put_run(struct disk *d, int trk, const unsigned char *p, long len)
{
 ...
 */
static int build_qdiv(unsigned char *out)
{
	unsigned q[2048];
	int p;
...
/*
 * vram.dat is u16 nchunks, then per chunk u24 addr, u16 len, data[len].  On the
 * disk that becomes a header sector (u8 nchunks, then per chunk u24 addr, u16
 * len) followed by each chunk's data, sector-aligned so load_vram can stream a
 * chunk with one ADDR0 setup.  Track 12 sector 0 IS that header: corrupt it and
 * the whole 57K upload is misdirected, which is why DSKERR names the sector.
 */
static long vram_sectors(const unsigned char *v, long vlen)
{
...
static long put_vram(struct disk *d, const unsigned char *v)
{
 ...
 * and rewrites operands at build time, so the raw .bin the assembler emits does
 * not match the disk.  These come from its A2VERA_DUMP_BLOBS hook.
 */
	       ...
	       char *err, size_t errsz)
{
	...
	if (boot1_len > LDR_PAGES * 256)
		FAIL("boot1 is %ld B but the loader is %d pages; the PROM page "
		     ...
		     "firmware gives up after ~6 sectors",
		     boot1_len, LDR_PAGES);
	...
	got = put_vram(d, vram);
	if (got != vsec)
		FAIL("VRAM sector count drifted (%ld written, %ld computed)",
		     got, vsec);
