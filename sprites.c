 ...
 *      b06  $30000   8K  BG tiles; the brick and vine injections come from
 */
#include <string.h>
...
static int
spx (const unsigned char *bank, int t, int x, int y)
{
...
static int
quad_px (const unsigned char *bank, int base, int px, int py)
{
  return spx (bank, base + (px >= 8) + 16 * (py >= 8), px & 7, py & 7);
}

static int
npx (const unsigned char *chr, int t, int x, int y)
{
...
static void
put8 (unsigned char *sheet, int slot, int x, int y, int lo, int hi)
{
...
static void
inj_nes (unsigned char *b07, const unsigned char *chr, int ntile,
...
static void
inj_smas8 (unsigned char *b07, const unsigned char *bank, int src, int slot)
{
  int x, y;

  for (y = 0; y < 8; y++)
    for (x = 0; x < 8; x += 2)
      put8 (b07, slot, x, y, spx (bank, src, x, y), spx (bank, src, x + 1, y));
}
 ...
 */
static void
inj_quad16 (unsigned char *b07, const unsigned char *bank, int base,
      ...
      for (x = 0; x < 16; x += 2)
        {
          ...
          b07[slot * 32 + y * 8 + x / 2] = (unsigned char) ((a << 4) | b);
        }
    }
}
...
static void
vera_pal (const unsigned char *rom, long off, unsigned char *out)
{
  unsigned x;
  int i, r, g, b;
  ...
  for (i = 0; i < 16; i++)
    {
      x = (unsigned) rom[off + i * 2] | ((unsigned) rom[off + i * 2 + 1] << 8);
      r = (x & 31) >> 1;
      g = ((x >> 5) & 31) >> 1;
      b = ((x >> 10) & 31) >> 1;
 ...
 */
void
build_palette (const struct rom *smas, unsigned char *out)
{
 ...
 */
void
build_font (const unsigned char *chr, unsigned char *out)
{
  int t, x, y;
    ...
    for (y = 0; y < 8; y++)
      for (x = 0; x < 8; x += 2)
        out[t * 32 + y * 4 + x / 2] = (unsigned char)
          ((npx (chr, 256 + t, x, y) << 4) | npx (chr, 256 + t, x + 1, y));
}

/* L0 boots as the blank/sky tile at palette offset 0, L1 as the same tile
 * transparent */
void
build_maps (unsigned char *l0, unsigned char *l1)
{
  int i;
...
/* the ring is idx2 */
static const unsigned char bubble_remap[4] = { 0, 0, 1, 0 };
...
/* four body quads plus the mouth-open head, remapped to VERA offset 13 */
 ...
 */
int
build_sprite_sheets (const struct rom *smas, const unsigned char *chr,
                     unsigned char *b0a, unsigned char *b07)
{
  ...
  for (i = 0; i < 5; i++)
    inj_quad16 (b07, sb07, bowser_base[i], bowser_slot[i], BOWSER_REMAP);
  return 0;
}
