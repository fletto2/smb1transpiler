 ...
 *
 * The port draws SMB1 using Super Mario All-Stars artwork.  None of that
 * may be redistributed, so this repo ships no Nintendo data: what it ships is
...
static int
slurp (const char *path, struct rom *r)
{
  ...
  if (!r->data || fread (r->data, 1, (size_t) r->size, f) != (size_t) r->size)
    {
      free (r->data);
      fclose (f);
      return 0;
...
static int
write_file (const char *dir, const char *name, const unsigned char *p, long n)
{
  char path[1200];
  FILE *f;
...
static int
is_smb1 (const struct rom *r, long *chr_off)
{
...
crc32b (const unsigned char *p, long n)
{
 ...
 */
static int
check_dumps (const struct rom *nes, const struct rom *smas, long chr_off)
{
  ...
  bad = (pc != CRC_SMB1_PRG) + (cc != CRC_SMB1_CHR)
    + (b6 != CRC_SMAS_B06) + (b7 != CRC_SMAS_B07) + (ba != CRC_SMAS_B0A);
  if (!bad)
    return 0;
...
/*
 * The L0 tileset.  SMAS keeps no ready-made 768-tile sheet anywhere in the
 * ROM: it DMAs different banks into different VRAM windows as it runs, so
 * the port's tiles_vera.bin is a snapshot of assembled VRAM and reproducing
 * it means replaying that mapping.  TILERULES has one rule per tile, in four
 * kinds, all of which read the caller's ROMs:
 *
 ...
 */
static int
build_tileset (const struct rom *smas, const struct rom *nes,
...
static int
varint_apply (const unsigned char *d, int count, unsigned char *img,
...
static int
apply_pairs (const unsigned char *offs, const unsigned char *vals,
...
/*
 * The game image: the caller's own PRG in three ordered stages.  The port
 * relocates SMB1 by -$7800 to $0800 and moves the PPU/APU register windows
 * into Language Card shadows, so every affected byte is an operand high byte
 * and the transform is positional: a2vera_blobs.h ships the offsets, the
 * bytes come from the ROM.  The generator that emitted those offsets measured
 * them by diffing an original-base assembly against the port's, and errors
 * out on any byte that fits no rule.
 *
 * The order matters in both directions, and getting it wrong is nearly
 * invisible.  The source edits are at the ORIGINAL base, so stage 2 must
 * rebase them; doing them afterwards left the vine-snap table 30K off.
 ...
 */
static int
build_game (const struct rom *nes, unsigned char *img)
{
  ...
  if (varint_apply (apu_deltas, APU_COUNT, img, 2, 32768))
    return -4;
  if (apply_pairs (gamepatch_off, gamepatch_val, GAMEPATCH_COUNT, img, 32768))
    return -5;
  return 0;
}
 ...
 */
static int
fill_vram (unsigned char *v, const struct rom *smas,
           const struct rom *nes, long chr_off, const unsigned char *tiles)
{
...
static int
build_disk (const struct rom *nes, const struct rom *smas,
            ...
            const char *vram_path, const char *outdir)
{
  ...
  ret = disk_build (&dsk, blob_boot1, sizeof (blob_boot1),
                    ...
                    vram, vram_len,
                    blob_lc_audio, sizeof (blob_lc_audio), err, sizeof (err));
  if (ret)
    {
...
static int
verify (const unsigned char *tiles, const char *ref_path)
{
  struct rom ref;
  ...
  for (blk = 0; blk < NTILES; blk += 64)
    {
      for (i = 0, ok = 0; i < 64; i++)
        if (!memcmp (tiles + (size_t) (blk + i) * VERA_TILE_BYTES,
                     ...
                     * VERA_TILE_BYTES, VERA_TILE_BYTES))
          ok++;
      total += ok;
  ...
  if (find_roms (&nes, &smas, &chr_off))
    {
      fprintf (stderr, "\nThis repository ships no Nintendo data by "
               ...
               "folder and the assets are derived from " "them.\n");
      ret = 1;
      goto err_free_roms;
    }
